...
{
	$is_exist = "select * from dress_style where name = '".$_POST['name']."'";
	$db_exist = $obj->select($is_exist);
	if(empty($db_exist))
	{
		$insert="INSERT into dress_style(id, name)values(NULL, '".$_POST['name']."')";
		$db_ins=$obj->insert($insert);	
		echo "<script>window.location='list_dressstyle.php'</script>";
	}
	else
	{
...
{	
	$is_exist = "select * from dress_style where name = '".$_POST['name']."' and id != '".$_GET['id']."'";
	$db_exist_role = $obj->select($is_exist);
	if(empty($db_exist_role))
	{
		$update_page="UPDATE dress_style SET name = '".$_POST['name']."' where id = '".$_GET['id']."'";
...
$select_category = "select * from dress_style where id = '".$_GET['id']."'";
                    ...
                    <div class="portlet-body form">
                        <form  method="post" id="form_sample_2" name="form_sample_3" class="form-horizontal" enctype="multipart/form-data">
                            ...
                            <div class="form-actions">
                                ...
                                <a href="list_dressstyle.php?did=<?php echo $_GET['id']; ?>" onclick="return delete_order('<?php echo $_GET['id']; ?>');"  class="btn red" >Delete</a>
                                <?php } ?>
                            </div>
                        </form>
                        <!-- END FORM-->
