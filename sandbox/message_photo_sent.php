...
<?php
	$select_new_msgs = "select photo_request.*, photo_request.Id as photo_request_id from photo_request,members where photo_request.From_mem_id = '".$_SESSION['logged_user'][0]['id']."' AND members.id=photo_request.From_mem_id AND members.is_profile_active='Y' order by photo_request.Id DESC";
	$messages = $obj->select($select_new_msgs);
?>
<?php
	for($i=0;$i<count($messages);$i++)
	{
		$sql2 = "SELECT members.*,members.id as mem_id,member_photos.photo,messages.id as msg_id,messages.* FROM messages,members LEFT JOIN member_photos ON members.id = member_photos.member_id where members.id = '".$messages[$i]['To_mem_id']."'";
