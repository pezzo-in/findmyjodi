...
class PAGING
{
	...
	*/
	function PAGING($sql,$records=5,$pages=9)
	{
		if($pages%2==0)
			$pages++;
		/*
	...
	function show_paging($url,$params="")
	{
		...
		if($this->total>$this->records)
		{
			...
			if($page_no==$previous)
			{
				//$paging.="<li class='paging-disabled'><a href='javascript:void(0)'>Pre</a></li>";
			}
			else
				$paging.="<li><a href='$url$params$previous'>Previous</a></li>";
			for($p=$start;$p<=$end;$p++)
			{
				$paging.="<li";
				if($page_no==$p)
			...
			if($page_no==$next)
			{
				...
				$paging.="<li><a href='$url$params$next'>Next</a></li>";
			//if($page_no==$last)
				//$paging.="<li class='paging-disabled'><a href='javascript:void(0)'>&gt;&gt;</a></li>";
			//else
