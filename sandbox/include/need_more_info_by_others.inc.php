...
?>
<?php		$select_new_msgs = "select * from need_more_info_detail
								where
  ...
  <div class="basicview">
    <div class="showbasiccontent">
      ...
      <div class="prfl-details">
        ...
        <br />
        <span class="smalltxt">Last Login : <?php echo date('d M, Y',strtotime($logged_in_member[0]['last_login'])); ?></span><br />
        <p><?php echo $each_msg[0]['age']; ?> Yrs <?php if(!empty($each_msg[0]['height'])) { echo ",Height ".$each_msg[0]['height']." Inch"; } ?> | 
			<?php if(!empty($each_msg[0]['religion']))
			{
				$select_religion = "select * from religions where id = '".$each_msg[0]['religion']."'";
				$religion = $obj->select($select_religion);
				echo $religion[0]['religion'];
			}?> 
			<?php
				if(!empty($each_msg[0]['caste']))
				{
					$select_caste = "select * from caste where id = '".$each_msg[0]['caste']."'";
					$caste = $obj->select($select_caste);
					echo ": ".$caste[0]['caste'];
			  	}
			?>
			  <?php
			  	if(!empty($each_msg[0]['city']))
				{ 
			  		echo " | Location : ".$each_msg[0]['city'];
				}
			  ?>
			  <?php 
			  		if(!empty($each_msg[0]['country']))
					{
						$select_country = "select * from mobile_codes where id = '".$each_msg[0]['country']."'";
						$country = $obj->select($select_country);
						echo ", ".$country[0]['country'];
					}
			  ?> 
			  <?php
			  		if(!empty($each_msg[0]['education']))
					{
			  	 		echo "| Education : ".$each_msg[0]['education'];
					}
					if($each_msg[0]['occupation'] != "" && $each_msg[0]['occupation'] != "Any" && $each_msg[0]['occupation'] != "Optional")
					{ 
						echo "Occupation : ".$each_msg[0]['occupation'];
					} ?></p>
              ...
              <div class="cb-msgs">
                ...
                <div class="cb-msgbox">
                  ...
                  <?php $replies_to_mem = "select * from replies 
															         where to_mem = '".$_SESSION['logged_user'][0]['member_id']."' and
																	 to_msg_id = '".$msg['id']."'";
...
<?php }
								...
								{
									echo "You have no any notification for need more information by anyone";
								} $j++; ?>
