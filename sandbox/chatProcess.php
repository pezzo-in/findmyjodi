		...
		if($res){
			...
			if($nbr==0){
				...
				foreach($data as $d){
					...
					$sql = "SELECT members.*,member_photos.photo, member_photos.Approve FROM members LEFT JOIN member_photos ON members.id = member_photos.member_id WHERE members.email_id='".$d[$emailColomn]."'";
					$sql_res=$db->query($sql);
					$sql_data=$db->retrieve_data($sql_res);
					
					$path = "upload/".$sql_data[0]['photo'];
					
					if(!empty($sql_data[0]['photo']) && $sql_data[0]['Approve']==1)
					{
						if (file_exists($path)) 
						{
							$avatarSrc=$path;
