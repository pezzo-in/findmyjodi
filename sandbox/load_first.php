							...
							<li class="morelink">
                                    <div class="searchby">
                                        <div class="searchbubblebox sbox-rel">                	
                                         ...
                                         <label><input type="checkbox" value="<?php echo $rel['religion']; ?>" name="Search_drpReligion[]" class="Search_drpReligion" <?php if($search_coockie_data['Search_drpReligion']==$rel['religion']) { ?> checked="checked"<?php } ?> onchange="refine_search()"/><?php echo $rel['religion']; ?></label>
											<?php } ?>
                                           </div>
                                    ...
                                    <div class="searchby">
                                        <div class="searchbubblebox sbox-rel">                	
										   ...
										   /*if($search_coockie_data['Search_drpReligion']!='') {
											...
											$caste_list="select * from caste where religion_id='".$db_religion[0]['id']."'";
											$db_caste=$obj->select($caste_list);
												for($i=0;$i<count($db_caste);$i++) { ?>
                                         <label><input type="checkbox" value="<?php echo $db_caste[$i]['caste']; ?>" name="drpCaste[]" <?php if($search_coockie_data['drpCaste']==$db_caste[$i]['caste']) { ?> checked="checked"<?php } ?> onchange="refine_search()"/><?php echo $db_caste[$i]['caste']; ?></label>
											<?php } } else { */
											
											 $all_caste_list="select * from caste order by religion_id";
											$all_caste=$obj->select($all_caste_list);
											for($i=0;$i<count($all_caste);$i++)  { ?>
<label><input type="checkbox" <?php if($search_coockie_data['drpCaste'] == $all_caste[$i]['caste']) { ?> checked="checked"<?php } ?> value="<?php echo $all_caste[$i]['caste']; ?>" name="drpCaste[]" onchange="refine_search()"/><?php echo $all_caste[$i]['caste']; ?></label>
                               				<?php } //} ?>
                                           </div>
                                           <div class="clear"></div>
                                        </div>
 ...
 <div class="mid_top_checkbox" style="clear: both;margin: 19px;float: right;margin-top: 0;"><span style="float:left; margin-right:15px; font-weight:bold"">Total Profiles : <?php echo count($members2); ?></span><a href="javascript:;" class="list_view">List View</a><a href="javascript:;" class="grid_view">Grid View</a></div><br clear="all" />     
        <input type="hidden" value="<?php echo count($members2); ?>" name="ttl_profile" id="ttl_profile">
         ...
         <ul class="profl-list" id="refine_data d">
            <?php
