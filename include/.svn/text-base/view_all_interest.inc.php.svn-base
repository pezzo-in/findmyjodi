...
//LOGGED-IN USER'S DETAIL //
	$sql = "SELECT express_interest.* , members.* , member_photos.photo
			FROM 
				express_interest, members, member_photos
			WHERE 
				express_interest.to_mem = members.member_id
			AND 
				members.id = member_photos.member_id
			AND express_interest.to_mem = '".$_SESSION['logged_user'][0]['member_id']."'";
...
<div class="mid_top">
        ...
        <?php if(!empty($messages)){ 
				foreach($messages as $msg) { 
					...
					$sql2 = "SELECT express_interest.to_mem,express_interest.from_mem, members . * , member_photos.photo
							FROM 
								express_interest, members, member_photos
							WHERE 
								express_interest.from_mem = '".$msg['from_mem']."'
							AND members.id = member_photos.member_id
							AND express_interest.to_mem = '".$_SESSION['logged_user'][0]['member_id']."'
							AND members.member_id = '".$msg['from_mem']."'";
							
							
						
					$get_each_msg = $obj->select($sql2);		
					?>
                    ...
                    <?php
						if(!empty($get_each_msg[0]['photo']))
						{
							...
							$path = "../upload/".$get_each_msg[0]['photo'];
							if (file_exists($path)) { 
                       			?><a href="view_profile.php?id=<?php echo $get_each_msg[0]['id']; ?>">
                    ...
                    <p><?php echo $get_each_msg[0]['name']; ?> ( <?php echo $get_each_msg[0]['member_id']; ?> )<br>
    <?php echo $get_each_msg[0]['religion']; ?><br /><?php echo $get_each_msg[0]['age']; ?></p>    
    			</div>
            </div>
					<?php foreach($get_each_msg as $get) {
					?>
        	<?php /*?><div class="profile_details">
                    ...
                    <p><?php echo $get['name']; ?> ( <?php echo $get['member_id']; ?> )<br>
    <?php echo $get['message']; ?></p>    
    			</div>
            </div><?php */?>
            <?php
				}
