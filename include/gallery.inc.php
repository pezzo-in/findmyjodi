...
if($_GET['id']!='')
{
	...
	$select_user_photo="select * from member_photos where member_id='".$_GET['id']."' AND Approve=1";
}
else
{
...
$img2=$obj->select($select_user_photo);
?>
	<?php if($_GET['id'] == '')
{
    ?>
...
<?php }else{ 
			  ...
			  ?> 
	<span style="font-size: 15px;font-weight: bold;padding: 10px; float:left;">This member have not uploaded any galley pictures yet.</span>
    <?php if($_GET['id']!=$_SESSION['logged_user'][0]['id'] && (count($img2)==0)) { ?><span id="success_msg" style="font-size: 15px;font-weight: bold;padding-top: 10px; float:left;"> Send a Photo request.</span>&nbsp;&nbsp;<a id="photo_button" onclick="send_photo('<?php echo $_SESSION['logged_user'][0]['id']; ?>','<?php echo $_GET['id']; ?>')" href="javascript:;"><img src="images/send_btn.png" alt="" /></a>
<?php } ?>
	<?php }else{ ?><span style="font-size: 15px;font-weight: bold;padding: 10px;">This member have not uploaded any pictures yet. <?php if($_GET['id']!=$_SESSION['logged_user'][0]['id'] && (count($img2)==0)) { ?>Photo request send</span><?php }}
