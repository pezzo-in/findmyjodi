...
\***************************************************************************/
$BLOGGER_APPID  = "http://www.blogger.com/api";
$BLOGGER_SERVER = "beta.blogger.com";
...
function blogger_newPost($blogid, $username, $password, $content, $title, $publish=false) {
	global $BLOGGER_APPID;
	...
	// Construct the actual request message
	$newPostRequest = new xmlrpcmsg("blogger.newPost", array($XMLappid, $XMLblogid, $XMLusername, $XMLpassword, $XMLcontent, $XMLtitle, $XMLpublish));
...
function blogger_editPost($postid, $username, $password, $publish=false, $content, $title) {
	global $BLOGGER_APPID;
	...
	$XMLpublish	= new xmlrpcval($publish, "boolean");
//	$content = "<title></title>";
	//$content = "<title>aaaaaaaaa</title>aaaaa";
	//die();
	$XMLcontent	= new xmlrpcval($content, "string");
	...
	$editPostRequest = new xmlrpcmsg("blogger.editPost", array($XMLappid, $XMLpostid, $XMLusername, $XMLpassword, $XMLcontent, $XMLpublish));
	//$editPostRequest = new xmlrpcmsg("blogger.editPost", array($XMLappid, $XMLpostid, $XMLusername, $XMLpassword, $XMLtitle, $XMLpublish));
...
function postnumber($postid)
	{
		...
		{	
			...
			$checkstr = substr($postid,$startstr,1);
		...
		$postchar = $postchar-1;
		$poststart = $len - $postchar;
		return substr($postid,$poststart,$postchar);
	}
/***************************************************************************\
				END OF LIBRARY
\***************************************************************************/
...
$TEST_USERNAME	= "admindogs@gmail.com"; // Plain-text username
$TEST_PASSWORD	= "admindog210"; // Plain-text password (don't freak out, just remove it befor going live!)
//$TEST_BLOG	= "3248931913025692035"; // A blog id to work with
//$TEST_POST	= "264308522913193230"; // A post id, Warning: this gets deleted if you test blogger_deletePost();
$TEST_NUM	= 15; // Change this to anything up to and including 20
$TEST_PUBLISH	= true; // true or false (or 1 or 0)
...
//$data = blogger_newPost($TEST_BLOG, $TEST_USERNAME, $TEST_PASSWORD, "hi", $TEST_PUBLISH);
//$data = blogger_editPost($TEST_POST, $TEST_USERNAME, $TEST_PASSWORD,$TEST_PUBLISH,"bye");
